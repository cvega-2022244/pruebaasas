...
export const updateUser =  async(req, res)=>{
    try {
        ...
        let updateUser = await Usuario.findOneAndUpdate(
            {_id: id}, 
            data, 
            {new: true} 
        )
...
export const deleteUser = async(req, res)=>{
    try {
        let { id } = req.params    
        let deletedUser = await Usuario.findOneAndDelete({_id: id})
...
export const assignUserCourse = async (req, res) => {
    try {
        ...
        const course = await Curso.findOne({ _id: courseId });
...
export const listUserCourses = async (req, res) => {
    try {
        let { userId } = req.uid;
        let user = await Usuario.findOne(userId).populate('courses');
